"""Individual-level multi-preference Double DQN-DE + DBSCAN-CMA-ES.

This is an experimental extension of ``online_individual_dqn_de_cmaes``.
Phase 1 keeps the existing individual KNN-DE search flow, but replaces the
single scalar-reward Double DQN with a shared multi-preference Double DQN.
The replay buffer stores reward vectors, so multiple Q-heads can reuse the
same online experience while learning coverage, quality, diversity, and
balanced search preferences.
"""
...
def run_optimizer(
    ...
    while fes < max_fes_total:
        ...
        action_ids = [agent.select_action(states[i], progress, rng) for i in range(np_size)]
        ...
        reward_vectors = _individual_reward_vectors(
            replacement_success,
            improvement,
            archive_gain,
            cluster_quality_gain,
            cluster_count_gain,
            parent_fit,
        )
    ...
    return {
        "algorithm": "online_individual_mpdqn_de_cmaes",
...
def _individual_reward_vectors(
    replacement_success: np.ndarray,
    improvement: np.ndarray,
    archive_gain: np.ndarray,
    cluster_quality_gain: np.ndarray,
    cluster_count_gain: float,
    parent_fit: np.ndarray,
) -> np.ndarray:
    """Build the 5-dimensional reward vector shared by all preference heads.

    Fixed dimension order:
    [archive_gain, fitness_improvement, diversity_gain, replacement_success, budget_efficiency]
    The algorithm layer does not pre-combine these terms; each Q-head applies
    its own scalarization weights inside MultiPreferenceDoubleDQNAgent.
    """
    ...
    diversity_component = np.tanh(cluster_quality_gain + float(np.tanh(cluster_count_gain)))
    success_component = np.asarray(replacement_success, dtype=float)

    # Each individual evaluates one trial per generation; this term mildly
    # penalizes ineffective evaluations and rewards useful ones.
    effective = (replacement_success > 0) | (archive_gain > 0) | (cluster_quality_gain > 0)
    budget_component = np.where(effective, 0.05, -0.05)
