"""在线个体级 Double DQN-DE + DBSCAN-CMA-ES 主算法。

与 niche-level 版本相比，本算法把 RL 控制粒度下沉到每个个体：
每个个体独立提取状态、独立选择 F / CR / 变异策略，并得到自己的 reward。
DBSCAN 小生境仍用于定义局部邻域和限制候选池，避免个体级策略破坏多峰隔离。
"""
...
def run_optimizer(
    ...
    while fes < max_fes_total:
        ...
        for i in range(np_size):
            agent.remember(states[i], action_ids[i], rewards[i], next_states[i])
    ...
    return {
        "algorithm": "online_individual_dqn_de_cmaes",
...
def _individual_rewards(
    ...
    imp_norm = np.tanh(improvement / fit_scale)
    rewards = (
        0.30 * replacement_success
        + 0.25 * imp_norm
        + 0.25 * np.tanh(archive_gain)
        + 0.15 * np.tanh(cluster_quality_gain)
        + 0.05 * float(np.tanh(cluster_count_gain))
    )
    ineffective = (replacement_success <= 0) & (archive_gain <= 0) & (cluster_quality_gain <= 0)
